...
button.addEventListener("click", (e) => {
  console.log(e);
  win_2 = document.querySelector(".end");
  win_2.className = "hidden";
...
function page() {
  ...
  const keyboard = [
    ...
    104, 106, 107, 108, 122, 120, 99, 118, 98, 110, 109,
  ];
  ...
  field_keyboard.addEventListener("click", function (e) {
    ...
    if (e.target.className === "key") {
      if (word_result.includes(e.target.textContent)) {
        ...
        document.querySelector(`.${man_parts[errors]}`).classList = document
          .querySelector(`.${man_parts[errors]}`)
          .className.split(" ")[0];
